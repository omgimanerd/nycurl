...
DataFormatter.format = function(data) {
  ...
  for (var article of articles) {
    var isGay = article.abstract.toLowerCase().indexOf('gay') != -1;
    var style = [];
    if (isGay) {
      style = ['rainbow'];
    }
    table.push([
      DataFormatter.applyStyles(article.section, ['underline', 'cyan']),
      DataFormatter.formatTextWrap(article.title, detailsWidth - 2, [
        'bold', 'cyan'
      ]) + '\n' +
      DataFormatter.formatTextWrap(article.abstract, detailsWidth - 2, style) +
